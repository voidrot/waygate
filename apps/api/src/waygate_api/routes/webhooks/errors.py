...
def map_dispatch_failure_to_http(
    ...
    """Map a workflow dispatch failure to an HTTP response.

    Args:
        result: The workflow dispatch result returned by the transport.

    Returns:
        A ``(status_code, detail)`` tuple suitable for ``HTTPException``.
    """
