...
- These docs describe the repository at its current structure: `apps/web`, `apps/scheduler`, `apps/draft-worker`, `apps/nats-worker`, `libs/core`, `libs/webhooks`, `libs/worker`, `libs/workflows`, and the plugins under `plugins/`.
