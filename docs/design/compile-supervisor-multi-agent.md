...
the `compile-supervisor-sequential` worktree.

Unlike the older planned-extension version of this doc, this branch now
...
| Pattern             | Use in this branch           | Why                                                              |
| ------------------- | ---------------------------- | ---------------------------------------------------------------- |
...
The compile agent layout in this branch is:
...
The branch now enforces the durable-versus-transient split from the migration
plan.
...
The branch now implements the optional later-stage guidance mechanism discussed
...
The current branch goes beyond simple accumulation of continuity state.
...
The main migration mechanics are implemented in this branch.
...
- decide whether source analysis should split continuity into finer specialist
  roles
- merge or otherwise reconcile this branch-local documentation with the main
